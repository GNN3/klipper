...
check_send_command(struct serialqueue *sq, int pending, double eventtime)
{
    ...
    if (! sq->ce.est_freq) {
        ...
        if (sq->ready_bytes)
            return PR_NOW;
        int ret = update_need_kick_clock(sq, 1);
        if (ret)
            return eventtime;
        return PR_NEVER;
    }
    ...
    int ret = update_need_kick_clock(sq, wantclock);
    if (ret)
                       ...
                       , struct list_head *msgs)
{
    ...
    if (list_empty(&cq->upcoming.msg_queue)) {
        list_add_tail(&cq->upcoming.node,
            &sq->transmit_requests.upcoming_queues);
        if (qm->min_clock < sq->transmit_requests.min_release_clock)
            sq->transmit_requests.min_release_clock = qm->min_clock;
